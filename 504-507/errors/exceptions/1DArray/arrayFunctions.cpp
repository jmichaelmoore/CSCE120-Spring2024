...
void loadRandom(int ary[], unsigned int size) {
  ...
  if (size >= CAPACITY) {
    throw std::out_of_range("size cannot be greater than " + CAPACITY);
  }
...
void insert(int val, unsigned int index, 
  ...
  // }
  if (index >= CAPACITY) {
    throw std::out_of_range("Cannot insert at location outside of capacity");
  }
...
void removeAtIndex(unsigned int index, 
          int ary[], unsigned int size) {
  if (index >= size) {
    throw std::out_of_range("index must be withing bounds of string");
  }
}
